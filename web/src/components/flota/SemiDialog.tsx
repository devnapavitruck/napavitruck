...
import {
  ...
  Grid, TextField, Button, MenuItem, IconButton, Tooltip, FormControlLabel, Switch
} from '@mui/material';
import DeleteOutlineIcon from '@mui/icons-material/DeleteOutline';

const TIPOS = ['Plataforma','Sider','Rampla','Portacontenedor','Frigorífico','Baranda','Tolva','Cisterna'];
...
export default function SemiDialog({ open, onClose, onSubmit }: Props) {
  const [f, setF] = React.useState({
    patente:'', tipo:'', marca:'', anio:'', descripcion:'',
    docPermisoArchivo:null as File|null, docPermisoVence:'',
    docRevisionArchivo:null as File|null, docRevisionVence:'',
    poseeSeguroCarga:false,
    docSeguroCargaArchivo:null as File|null, docSeguroCargaVence:'',
  ...
  const validate=()=>{
    ...
    if (!f.tipo) e.tipo='Tipo obligatorio';
    if (!f.patente) e.patente='Patente obligatoria';
    if (f.descripcion && f.descripcion.length>150) e.descripcion='Máx. 150 caracteres';
  ...
  const submit=()=>{
    ...
    formData.append('poseeSeguroCarga', String(f.poseeSeguroCarga));
    if (f.poseeSeguroCarga && f.docSeguroCargaArchivo) formData.append('docSeguroCargaArchivo', f.docSeguroCargaArchivo);
    if (f.poseeSeguroCarga && f.docSeguroCargaVence) formData.append('docSeguroCargaVence', f.docSeguroCargaVence);
        ...
        <Grid container spacing={2} sx={{ mt:.5 }}>
          <Grid item xs={12} md={4}>
            <TextField
              ...
              fullWidth
              error={!!err.tipo} helperText={err.tipo}
            >
          ...
          <Grid item xs={12} md={8}>
            <TextField
              label="Descripción (opcional, máx. 150)"
              value={f.descripcion}
              onChange={(e)=>set('descripcion', e.target.value)}
              fullWidth
              error={!!err.descripcion} helperText={err.descripcion}
            />
          </Grid>

          {/* Permiso */}
          <Grid item xs={12}><strong>Permiso</strong></Grid>
          ...
          {/* Seguro de carga (si posee) */}
          <Grid item xs={12}>
            <FormControlLabel
              control={<Switch checked={f.poseeSeguroCarga} onChange={(e)=>set('poseeSeguroCarga', e.target.checked)} />}
              label="Posee seguro de carga"
            />
          </Grid>
          {f.poseeSeguroCarga && (
            <>
              <Grid item xs={12} md={6}>
                <TextField
                  ...
                  onChange={(e)=> {
                    const file = (e.target as HTMLInputElement).files?.[0] ?? null;
                    set('docSeguroCargaArchivo', file);
                  }}
                  label="Archivo"
                />
              </Grid>
              <Grid item xs={12} md={6}>
                <TextField
                  type="date" label="Vence" fullWidth InputLabelProps={{shrink:true}}
                  value={f.docSeguroCargaVence}
                  onChange={(e)=>set('docSeguroCargaVence', e.target.value)}
                />
              </Grid>
            </>
          )}
